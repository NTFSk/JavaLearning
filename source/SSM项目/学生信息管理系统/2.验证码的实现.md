...
![image-20200424142345546](验证码文件.png)



* 提供组成验证码的字符数组，字体，字体样式

* 用于设置验证码长度，图片字体大小，图片高度，图片宽度，干扰线条数的私有属性

* 构造函数：无参、、一个参数：设置验证码长度、、三个参数：指定验证码长度、图片宽度、高度

* 用于生成字符串验证码的方法 `String generatorVCode()`

* 用于生成验证码图片的方法 ` BufferedImage generatorVCodeImage(String vcode, boolean drawline)`

* 用于生成旋转字体验证码图片的方法 `BufferedImage generatorRotateVCodeImage(String vcode, boolean drawline)`

* 用于给验证码图片画干扰线的方法 `void drawDisturbLine(Graphics g)`，会在生成图片的方法中被调用

* 获取一张旋转的图片 `BufferedImage getRotateImage(char c)`
